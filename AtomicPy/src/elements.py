...
def asymb_eln(ASYMB):
    ...
    for i in range( len(ASYMB) ):
        eln_found = 1
        for eln_n in range( len(elsymbol)):
            if( elsymbol[eln_n] == ASYMB[i].strip() ):
                ELN.append( eln_n )
                eln_found = 0
                break
        if( eln_found ):
...
def mass_asymb(AMASS):
    ...
    for i in range( len(AMASS) ):
        mass = AMASS[i]
        m_round = int(mass)
        for m_indx in range( len(elmass) ):
            mref_round = int(elmass[m_indx])   
            if( m_round == mref_round ):
                el_indx = m_indx
                break
        if(debug): print mass,m_round,mref_round,elsymbol[el_indx]
        ASYMB.append( elsymbol[el_indx] )
        ELN.append( int(el_indx) )
...
def set_elmass():
    ...
    ELMASS = numpy.zeros(n_elements+1)
...
def covalent_radi():
    ...
    radi_cov = numpy.zeros(n_elements+1)
...
def vdw_radi():
    ...
    radi_vdw = numpy.zeros(n_elements+1)
