		...
		for cent_angle in range(cent_min,cent_max,cent_step):
		    ...
		    run_qm = gaussian.check_fchk(fchk_file)
                    ...
                    if(run_qm ):
                        ...
                        job_dir = struct_dir+"/"+calc_id
                        os.chdir(job_dir)
                        
                
                        # Fix random bug issue of a space in the .chk file
                        bad_chk_file = job_name  + '-\ ' + dih_id + '_' + str(cent_angle) + '_auxfix'+".chk"
                        chk_file = calc_id+".chk"
                        
                        mv_chk = "mv "+bad_chk_file + " " + chk_file
                        print mv_chk
                        os.system(mv_chk)
                        
                        frm_chk = "formchk "+ chk_file
                        os.system(frm_chk)
