...
def main():
    ...
    for indx_file in args:
		...
		if( metadata_found  ):
			...
			for dih_indx in range( len(qm_dih_id_list) ):
			    ...
			    plot_l =  " \'"+dih_qm+"\' " + ' us 2:($3- ' + str(qm_min) + ') w l ls '+str(calc_i+1)+'  title '+ "\' " + mol_id+"n="+str(n_units)+" dihedral "+dih_id+" \' smooth unique  , \\" + "\n"
    ...
    for indx_file in args:
        ...
        for line in Lines:
            ...
            if( len(col) >= 4 and col[0] == "gen" ):
                ...
                mol_dir = col[1].strip()
                mol_id = col[2].strip()
                n_units = int(col[3].strip() )
                mol_acc = col[4].strip()
                
                # File info
                struct_dir = mol_dir + "/" + mol_id + "/"
                job_name = mol_acc + "_" + mol_id + "_n" + str(n_units)
		    ...
		    b_dat = open(minmax_data ,"w")			
		    b_dat.write("# mol_i , n_units , dih_id, qm_min, ang_min, qm_max, ang_max , qm_bar " )
		    b_dat.close()
		    
		    zerobar_data = struct_dir +'/' + mol_acc + "_" + mol_id + options.qm_sufix + '_bar.dat'
		    b_dat = open(zerobar_data ,"w")	
    		    b_dat.write("# n_units , d_indx - n_max, qm_bar " )
		    b_dat.close()		    
		...
		bar_cnt[mol_i,n_units] += 1
            ...
            if( len(col) >= 4 and col[0] == "ff_dih"  ):
                ...
                for m_indx in range(len(mol_list)):
                    ...
                    for calc_indx in range(len(mdir_list[m_indx])):
                        method_id = mdir_list[m_indx][calc_indx]
                        mol_dir = moldir_list[m_indx][calc_indx]
                        n_units = repeat_list[m_indx][calc_indx]
			...
			job_name = mol_acc + "_" + mol_id + "_n" + str(n_units)
                        ...
                        #job_name_ref = mol_id_ref + "_n" + str(n_units)
	...
	for n_units in range( n_max ):
	    f.write("\n")
	    for dih_indx in range( n_max*2 ):
		b_val = bar_grid[n_units,dih_indx]
		f.write( "\n %d %d %f " %  ( n_units, dih_indx, b_val) )
