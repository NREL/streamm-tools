...
def get_options():
    ...
    parser.add_option("--hist_out", dest="hist_out", type="string", default="angles.hist", help="Output historgam file ")
...
def main():
	...
	if( add_i ):
		...
		if( add_j ):
			...
			if( add_k ):
			    angle_list.append( [atom_k,atom_i,atom_j] )
			    
			    if(debug):
				print " found angle ",atom_k,atom_i,atom_j
	...
	if( file_io.file_exists( frame_id ) ):
		...
		bin_index = int( round( angle_kij/options.bin_size) )
		hist_cnt[bin_index] += 1
		
		if( options.verbose ):
		    print " # ",a_k,a_i,a_j,angle_kij
