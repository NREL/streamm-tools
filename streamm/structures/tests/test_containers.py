...
class TestContainer(unittest.TestCase):
    ...
    def test_bonds(self):
        ...
        self.bond_i = bonds.Bond(0,2)
        bond_str.append(' 0 - 2')
        self.strucC.add_bond(self.bond_i)
        self.bond_i = bonds.Bond(0,3)
        bond_str.append(' 0 - 3')
        self.strucC.add_bond(self.bond_i)
        self.bond_i = bonds.Bond(0,4)
        bond_str.append(' 0 - 4')
        self.strucC.add_bond(self.bond_i)
        self.bond_i = bonds.Bond(0,5)
        bond_str.append(' 0 - 5')
        self.strucC.add_bond(self.bond_i)
        self.bond_i = bonds.Bond(0,6)
        bond_str.append(' 0 - 6')
        self.strucC.add_bond(self.bond_i)
        self.bond_i = bonds.Bond(1,7)
        bond_str.append(' 1 - 7')
        self.strucC.add_bond(self.bond_i)
        self.bond_i = bonds.Bond(1,8)
        bond_str.append(' 1 - 8')
        self.strucC.add_bond(self.bond_i)
        self.bond_i = bonds.Bond(1,9)
        bond_str.append(' 1 - 9')
        self.strucC.add_bond(self.bond_i)
...
class TestBuildThiophene(unittest.TestCase):
    ...
    def test_calcdihc(self):
        dih_i = dihedrals.Dihedral(3,2,1,0)
    ...
    def test_writestruc(self):
        ...
        for pkey_i, particle_i  in self.strucC.particles.iteritems():
            pos_i = self.strucC.positions[pkey_i]
            particle_line = "  %s %f %f %f  "%(particle_i.tag,float(pos_i[0]), float(pos_i[1]),float(pos_i[2]))
...
class Testiadd(unittest.TestCase):
    ...
    def test_join(self):
        ...
        cnt = 0
        for pkey_i, particle_i in self.strucC.particles.iteritems():
            self.assertEqual(pos_str_list[pkey_i],' %d - %s %f %f %f '%(pkey_i,particle_i.tag,self.strucC.positions[pkey_i][0],self.strucC.positions[pkey_i][1],self.strucC.positions[pkey_i][2]))
            for pkey_j in   self.strucC.bonded_nblist.getnbs(pkey_i):
                #print " str_nbs_list.append(\' %d - %d \')"%(pkey_i,pkey_j)
                self.assertEqual(str_nbs_list[cnt],' %d - %d '%(pkey_i,pkey_j) )
                cnt += 1
