...
def get_normal_from_PCA(atompos):
    ...
    # The normal is the eigenvector for the smallest eigenvalue.  Get this from Cayley-Hamilton
...
# keeps track of a list of atoms
class Structure:
...
#        print "deleting link for atom index ", idx
    ...
    def get_normal_old(self, root_idx = 0):
        # assume structure is mostly flat, so 3 non colinear points defines it.
        natoms = len(self.atoms)
        idx2 = (root_idx+1) % natoms
        while (idx2 == root_idx):
            idx2 = (idx2 + 1 ) % natoms
        v1 = vec_sub(self.atoms[idx2].pos, self.atoms[root_idx].pos)
        idx3 = (idx2+1)%natoms
        while (idx3 == root_idx or idx3 == idx2):
            idx3 = (idx3+1)%natoms
        v2 = vec_sub(self.atoms[idx3].pos, self.atoms[root_idx].pos)
        while (vec_norm(vec_cross(v1,v2)) == 0):
            v2 = vec_sub(self.atoms[idx3].pos, self.atoms[root_idx].pos)            
            idx3+= 1
            if (idx3 >= len(self.atoms)):
                print "in get_normal, all points colinear, picking arbitrary normal"
                v2[0] = v1[0] + 5.0
                v2[1] = v1[1] + 5.0
                v2[2] = v1[2] + 5.0
                break

#        print "get normal, used indices: ", root_idx, idx2, idx3
#        v3 = vec_sub(self.atoms[2].pos, self.atoms[1].pos)
#        i = 3
#        v4 = vec_sub(self.atoms[i].pos, self.atoms[1].pos)
#        while (vec_norm(vec_cross(v3,v4)) == 0):
#            v4 = vec_sub(self.atoms[i].pos, self.atoms[1].pos)
#            i+= 1
...
#        print v1,v2,n1
        n = vec_normalize(n1)
#        n = vec_normalize(vec_add(n1,n2))
#        print "get normal: ", self.atoms[0].pos, self.atoms[1].pos, self.atoms[2].pos, v1, v2, n1
...
class Fragment(Structure):
    ...
    def get_connectors_old(self, term_idx, term_tag, cap_tag):
        import re
...
#                self.atoms[i].link.dump()
            if (self.atoms[i].link != None and self.atoms[i].link.type in term_tag):
                if (termcnt == term_idx):
                    term_atom_idx = i
                    break
                termcnt+=1
        ...
        if (term_atom_idx == None):
            print "term_idx %d not found in fragment, expect crash!" % (term_idx)
...
#                self.atoms[i].link.dump()
            if (self.atoms[i].link != None and self.atoms[i].link.type in cap_tag):
                if (self.atoms[i].link.targetpos == term_atom_idx):
                    # found correct cap
                    cap_atom_idx = i
                    break
        if (cap_atom_idx == None):
            print "cap for term_idx %d not found in fragment, expect crash!" % (term_idx)
...
##        print "equal ?",  term_atom_idx, self.atoms[cap_atom_idx].link.targetpos
    ...
    def find_neighbors(self, exclude=[]):
        ...
        # com is "center of mass", will be used to place oxygens at end
...
#            print "alt pat:", rot_ctrl.alternation_pattern
 ...
 #       print "counter ", rot_ctrl.counter
 #       print "free axis ", free_axis
 #       print "free_axis_rot ", free_axis_rot
