...
class CoordUtilsTest(unittest.TestCase):
    ...
    def test_find_in_coord_list(self):
        ...
        nptu.assert_equal(find_in_coord_list(coords, test_coord,
                                                 atol=0.15), [0, 2])
    ...
    def test_all_distances(self):
        ...
        nptu.assert_almost_equal(all_distances(coords1, coords2), result, 4)

    def test_pbc_diff(self):
        nptu.assert_almost_equal(pbc_diff([0.1, 0.1, 0.1], [0.3, 0.5, 0.9]),
                                    [-0.2, -0.4, 0.2])
        nptu.assert_almost_equal(pbc_diff([0.9, 0.1, 1.01],
                                             [0.3, 0.5, 0.9]),
                                    [-0.4, -0.4, 0.11])
        nptu.assert_almost_equal(pbc_diff([0.1, 0.6, 1.01],
                                             [0.6, 0.1, 0.9]),
                                    [-0.5, 0.5, 0.11])
        nptu.assert_almost_equal(pbc_diff([100.1, 0.2, 0.3],
    ...
    def test_pbc_shortest_vectors(self):
        ...
        nptu.assert_almost_equal(dists, expected, 3)
...
class SimplexTest(unittest.TestCase):
    ...
    def test_2dtriangle(self):
        ...
        nptu.assert_almost_equal(s.bary_coords([0.5, 0.5]),
                                    [0.5, 0, 0.5])
        nptu.assert_almost_equal(s.bary_coords([0.5, 1]), [0.5, 0.5, 0])
        nptu.assert_almost_equal(s.bary_coords([0.5, 0.75]), [0.5, 0.25, 0.25])
        nptu.assert_almost_equal(s.bary_coords([0.75, 0.75]), [0.25, 0.5, 0.25])
    ...
    def test_bary_coords(self):
        ...
        nptu.assert_almost_equal(bc, [0.26, -0.02, 0.76])
        new_point = s.point_from_bary_coords(bc)
        nptu.assert_almost_equal(point, new_point)
    ...
    def test_intersection(self):
        ...
        expected = np.array([[1.13333333, 0.06666667],
                             [ 0.8,  0.4]])
        nptu.assert_almost_equal(intersections, expected)
