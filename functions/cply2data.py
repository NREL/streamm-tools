...
def get_options():
    ...
    # Reference file
    #
    parser.add_option("--tag", dest="tag", type="string", default="groups", help="")    
    #parser.add_option("--ref", dest="ref", default="", help=" Reference json file to track json files ")
    ...
    parser.add_option("--LPimp", dest="LPimp", default=False,action="store_true", help="Put impropers on ON attached to LP's ")
    #
...
def read_param( param_file,verbose = True, debug = False):    
    """
    Read parameter file 
    ...
    BONDS
        V_BOND = k_f ( r -r_o )^2
        i j     k_f ( kcal mol^-1 A^-2 )  r_o (A)
    ANGLE
        V_ANGLE = k_f ( theta - theta_o )^2
        i j  k   k_f ( kcal mol^-1 rad^-2 )  theta_o (degree)
    DIHEDRAL
        V_DIHEDRAL = V_n/2 [ 1 + cos( n phi - gamma ) ]
        k i j l  No. Paths  Vn/2 ( kcal mol^-1  )  phi (degree) n 
    IMPROPER
        V_IMPROPER = ?? # V_n/2 [ 1 + cos( n phi - gamma ) ]
        k i j l   Vn/2 ( kcal mol^-1  )  phi (degree) n 
    """
    ...
    for line in lines:
        ...
        if( read_Pair ):
            if( len(col) >= 3 ):
                ...
                sigma = float(col[1])*2.0 
                epsilon = float(col[2])
                ljtyp_i = parmC.ljtypC[cnt_Pair]
        ...
        if( read_Bond_coeff ):
            if(  len(col) >= 3 ):
                ...
                btype = "harmonic"
                g_type = 1
                btyp_i = bondtype(str(col[0]),str(col[1]),btype)
                btyp_i.setharmonic(r0,kb)
                btyp_i.set_g_indx(g_type)
                btyp_i.set_lmpindx(cnt_Bond_coeff)
        ...
        if( read_Angle_coeff ):
            if(  len(col) >= 3 ):
                ...
                atyp_i = angletype(str(col[0]),str(col[1]),str(col[2]),atype)
                atyp_i.set_g_indx(gfunc_type)
                atyp_i.set_lmpindx(cnt_Angle_coeff)
        ...
        if( read_Dihedral_coeff ):
            if( len(col) >= 3 ):
                ...
                dtyp_i = dihtype(str(col[0]),str(col[1]),str(col[2]),str(col[3]),dtype)
                dtyp_i.set_g_indx(gfunc_type)
                dtyp_i.set_lmpindx(cnt_Dihedral_coeff)
                dtyp_i.setharmonic(n , kb,d)
                dtyp_i.setpaths(paths)
        ...
        if( read_Improper_coeff ):
            if(  len(col) >= 3 ):
                ...
                n = float(col[6])
                # i j k l 
                imptyp_i = imptype(str(col[0]),str(col[1]),str(col[2]),str(col[3]),dtype)
                imptyp_i.set_g_indx(g_indx)
                imptyp_i.setimp(Eo,KE)
                imptyp_i.set_lmpindx(cnt_Improper_coeff)
...
def set_param(struc_o,param_all,norm_dihparam,cov_nblist, cov_nbindx,check_paths=True,verbose=True ):
    """
    ...
    imp_cnt = 0
    debug_check_paths = False  
    if( check_paths ):
        Bonded_paths = [None]* len(struc_o.bondC)
        Bondids_dihs = []
    for d_o,dihObj_o in dihC_o:
        ...
        if( check_paths ):
            b_i,Bond_i  =  struc_o.bondC.find_ids(pid_i,pid_j)
            if( b_i not in Bondids_dihs ):
                Bondids_dihs.append(b_i)
            if(  Bonded_paths[b_i-1] == None ): Bonded_paths[b_i-1] = []
        if( debug):
        ...
        for dtyp_p, dtypObj_p  in dtypC_p:
            ...
            if( fftype_k == p_k  and  fftype_i == p_i  and  fftype_j == p_j  and  fftype_l == p_l ):
                ...
                if( check_paths ):
                    Bonded_paths[b_i-1].append( dtypObj_p.getpaths() )
                    if( debug_check_paths ):
                        print " Setting No. paths for bond {} between particles {} and {} to {} ".format(b_i,pid_i,pid_j,dtypObj_p.getpaths())
        ...
        if( new_type ):
            ...
            if( not type_found ):
                if(debug):
                    ...
                    print "   looking for ",fftype_k,fftype_i,fftype_j,fftype_l 
                copy_type = False 
                for d_all, dtypObj_all  in dtypC_all:
                    all_k = dtypObj_all.ptype1 
                    all_i = dtypObj_all.ptype2 
                    ...
                    all_l = dtypObj_all.ptype4
                    if( debug ):
                       print  " Checking match ",all_k,all_i,all_j,all_l
            ...
            if( type_found ):
                ...
                if( check_paths ):
                    b_i,Bond_i  =  struc_o.bondC.find_ids(pid_i,pid_j)
                    Bonded_paths[b_i-1].append( dtypObj_temp.getpaths() )
                    if( debug_check_paths ):
                        print " Setting No. paths for bond {} between particles {} and {} to {} ".format(b_i,pid_i,pid_j,dtypObj_temp.getpaths())
                        print dtypObj_temp
    ...
    if( check_paths ):
        for b_o in Bondids_dihs:
            # print b_o,Bonded_paths[b_o-1]
            for paths in Bonded_paths[b_o-1]:

                if( len(Bonded_paths[b_o-1]) <  paths   ):
                    error_line = " Incorrect number of paths for bond {} \n".format(b_o)
                    error_line += " {} dihedrals found which is less than the number of paths used in the paramterization {} \n".format(len(Bonded_paths[b_o-1]) ,  paths)

                    error_line += " {}  ".format(bondC_o[b_o])
                    sys.exit(error_line)
    #
    ...
    for imp_o,impObj_o in impC_o:
        ...
        if( new_type ):
            ...
            if( cnt_check < 1 ):
                print " No Imp Dih parameters were found for dih type %s-%s-%s-%s "%(fftype_k,fftype_i,fftype_j,fftype_l)
                raise TypeError
            elif( cnt_check > 1 ):
                print " %d Imp Dih parameters were found for dih type %s-%s-%s-%s please check parameter file  "%(cnt_check,fftype_k,fftype_i,fftype_j,fftype_l)
...
def write_data(strucC,parmC,data_file):
    ...
    if( len(parmC.dtypC) > 0 ):
        ...
        for dtyp_p, dtypObj_p  in parmC.dtypC:    
            if( dtypObj_p.get_type() == "multiharmonic"):
                ...
                p = dtypObj_p.getpaths()
                w = 0.0 # Weight 
                F.write( "%10d %12.6f %d  %d %12.6f # %d x %5s %5s  %5s  %5s   \n" % (dtyp_p-imp_cnt,K,n,d,w, p, dtypObj_p.get_ptype1(),dtypObj_p.get_ptype2(),dtypObj_p.get_ptype3(),dtypObj_p.get_ptype4()  ) )
    ...
    if( len(strucC.angleC) > 0 ):
        ...
        for a_o, angleObj_o in strucC.angleC:
            ...
            AT_k = strucC.ptclC[ a_k ].tagsDict["fftype"]
            AT_i = strucC.ptclC[ a_i ].tagsDict["fftype"]
            AT_j = strucC.ptclC[ a_j ].tagsDict["fftype"]

            
            
            F.write(  '%9d %8d %9d %9d %9d  # %s %s %s \n' % (a_cnt,a_ind,a_k,a_i,a_j,AT_k,AT_i,AT_j) )
        F.write(  '\n' )
    ...
    if( len(strucC.dihC) > 0 ):
        ...
        for d_o,dihObj_o in strucC.dihC:
            ...
            AT_j = strucC.ptclC[ d_j ].tagsDict["fftype"]
    ...
    if( len(strucC.impC) > 0 ):
        ...
        for d_o,impObj_o in strucC.impC:
            if( impObj_o.get_type() == "improper"   ):
                ...
                AT_j = strucC.ptclC[ d_j ].tagsDict["fftype"]
                AT_l = strucC.ptclC[ d_l ].tagsDict["fftype"]
                F.write(  '%9d %8d %9d %9d %9d %9d # %s %s %s %s \n' % (imp_cnt,imp_ind,d_k,d_i,d_j,d_l,AT_k,AT_i,AT_j,AT_l) )
            else:
...
def set_LPimporpers(strucC,cov_nblist, cov_nbindx,lmpindx=1,debug=False):
    """
    Add impropers around ON's with two LP's 
    """
    if( debug): print " Setting LP imps in strucC ",len(strucC.ptclC)
    for pid_i, ptclObj_i  in strucC.ptclC:
        
        if( debug): print  ptclObj_i
            
        if( ptclObj_i.tagsDict["fftype"] == 'ON' ):
            i_o = pid_i
            LPlist = []
            if( debug): print " ON found ",i_o
                
            for pid_j in cov_nblist[ cov_nbindx[ pid_i]:cov_nbindx[ pid_i + 1 ] ]:
                if( strucC.ptclC[pid_j].tagsDict["fftype"] == 'LP' ):
                    LPlist.append(pid_j)
                else:
                    k_o = pid_j 
            if( len(LPlist) == 2 ):
                if( debug):
                    print " found ON bonded to two LP's "
                    print k_o,i_o, LPlist[0],LPlist[1] 
                impObj = Improper( i_o,k_o, LPlist[0],LPlist[1] )
                impObj.set_lmpindx(lmpindx)
...
def main():
    ...
    # oplsaa_atomtypes(bb_o,bb_o.bonded_nblist, bb_o.bonded_nbindx)
    if( options.in_itp ):
        if( options.verbose ): print " Reading itp file {} ".format(options.in_itp)
    ...
    if( len(bb_o.bondC) > 0 ):
        if( options.verbose ): print " {} bonds found  ".format(len(bb_o.bondC))
        bb_o.bondC_nblist()
    else:
        if( options.verbose ): print " No bond found creating bonded niehgborlist based on radii "
        bb_o.build_bonded_nblist(max_nn=12.0,radii_buffer=1.25)
        bb_o.nblist_bonds()            
    bb_o.nblist_angles()
    bb_o.nblist_dih(debug=False)
    if( options.LPimp ):
        if( options.verbose ):
            print " Setting LP impropers "
        set_LPimporpers(bb_o,bb_o.bonded_nblist, bb_o.bonded_nbindx,debug=True)

    print bb_o
    

    for pid_i, ptclObj_i  in bb_o.ptclC:
        if(  ptclObj_i.tagsDict["fftype"] == "??" ):
            if( options.verbose ):
                logline = " Unknown fftype {} for particle {} type {} \n".format(ptclObj_i.tagsDict["fftype"],pid_i,ptclObj_i.type)
                logline += " will guess fftypes "
                print logline 
            oplsaa_atomtypes(bb_o,bb_o.bonded_nblist, bb_o.bonded_nbindx)
            break
        
    if( options.verbose ): print " Setting parameter  "
    norm_dihparam = False
    param_i,bb_o  = set_param(bb_o,param_o,norm_dihparam,bb_o.bonded_nblist, bb_o.bonded_nbindx,check_paths=False)

    if( options.verbose ):
        print bb_o
    
    if( options.verbose ): print " Writing data file {} ".format(options.out_data)
    write_data(bb_o,param_i,options.out_data)

    comment = " From cply file {} ".format( options.in_cply)
    append = False 
    bb_o.ptclC.write_xmol("{}.xyz".format(options.out_data),comment,append)
