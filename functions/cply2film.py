...
def get_options():
    ...
    #
    parser.add_option("--ranmol", dest="ranmol", type="string", default="", help="Input for molecules to be randomly placed ")
    parser.add_option("--grid_cply", dest="grid_cply", type="string", default="", help="Input for grid molecule .cply  file")
    parser.add_option("--grid_n", dest="grid_n", type=int, default=0, help="Numnber times grid molecule is to be replicated  ")
    parser.add_option("--perc_grid", dest="perc_grid", type=float, default=0.0, help="Percent grid by mass ")
    #
    #
    parser.add_option("--seed", dest="seed", type="int", help="Seed for random")
    parser.add_option("--atomic_cut", dest="atomic_cut", type=float, default=4.0, help="Minimum distance between atoms of molecules ")
    parser.add_option("--lc_expand", dest="lc_expand", type=float, default=0.200, help="Fraction of the box size to increase system size after max_sys is excieded ")
    parser.add_option("--max_sys", dest="max_sys", type=int, default=3, help="Maximum system recreations at a certain lattice constant ")
    parser.add_option("--max_mol_place", dest="max_mol_place", type=int, default=50, help="Maximum attempts to place a molecule  ")
    ...
    parser.add_option("--min_grid_len", dest="min_grid_len", default=False,action="store_true", help=" Use solvent length, buffer and number to determine minimum box size ")
    parser.add_option("--in_param", dest="in_param", type="string", default="", help="Input  .prama file hopefully similar to AMBER parameter file ")
    parser.add_option("--lat", dest="lat", type="float", default=0.0, help="  initial lattice vector length ")
...
def read_data( strucC , parmC , data_file, verbose = True, debug = False):
    """
...
def add_struc(verbose,p,rank,molecule_rep,struc_i,n_struc_i,atomic_cut,lc_expand,max_sys,max_mol_place,calc_overlap,seed,debug = False):
    """
    ...
    if( len(molecule_rep.ptclC) > 0 ):
        for pid, ptclObj in molecule_rep.ptclC :
            if( ptclObj.tagsDict["chain"] > max_rep_chain ):
                max_rep_chain  = ptclObj.tagsDict["chain"]

    max_i_chain = 0
    min_i_chain = 100000000
    for pid, ptclObj in struc_i.ptclC :
        if( ptclObj.tagsDict["chain"] < min_i_chain ): min_i_chain =ptclObj.tagsDict["chain"]
        if( ptclObj.tagsDict["chain"] > max_i_chain ): max_i_chain =ptclObj.tagsDict["chain"]

    range_i_chain = max_i_chain - min_i_chain + 1
    # Set chain number to  max_rep_chain - range_i_chain
    #   so each additional chain
    #   chain += range_i_chain
    #   c_1 = c_o + range_i_chain = max_rep_chain + 1
    #   c_o = max_rep_chain + 1 - range_i_chain
    # only if c_o  [ 0:(N-1) ] 
    
    for pid, ptclObj in struc_i.ptclC :
        #Set chains to 0:(N-1) ptclObj.tagsDict["chain"] +=   - min_i_chain 
        # set to the max from previous  ptclObj.tagsDict["chain"] += max_rep_chain + 1 
        # Subtract range which will be added back on first placement ptclObj.tagsDict["chain"] +=   -range_i_chain
        # ptclObj.tagsDict["chain"] += - min_i_chain +  max_rep_chain + 1 -range_i_chain
        # ptclObj.tagsDict["chain"] +=  - min_i_chain +  max_rep_chain + 1 - max_i_chain + min_i_chain - 1
        # ptclObj.tagsDict["chain"] +=    max_rep_chain  - max_i_chain 
        ptclObj.tagsDict["chain"] +=   max_rep_chain  - max_i_chain 

    debug_chain = False
    if(debug_chain):

        print " in add_struc max_rep_chain ",max_rep_chain
        print " in add_struc max_i_chain ",max_i_chain
        print " in add_struc max_i_chain ",max_i_chain
        print " in add_struc range_i_chain ",range_i_chain
        sys.exit("max_rep_chain")
    ...
    if( debug ):
        print rank, " splitOnProcs "
    # Create a list of atomic indices for each processor 
    myChunk  = p.splitListOnProcs(pointIndices)

    if( verbose and len(myChunk) > 0 ):
        proc_out = " Proc {} has atoms {} - {} ".format(rank,myChunk[0],myChunk[-1])
        print proc_out
    
    #
    # Start adding molecules to the syif( stem
    #
    add_mol = True
    if( rank == 0  ):
        log_line = " Adding %d  molecules  "%n_struc_i
        log_line += "   into box  {} {} {} \n".format(molecule_rep.getLatVec()[0][0],molecule_rep.getLatVec()[1][1],molecule_rep.getLatVec()[2][2])
        print log_line
        if( seed == None ):
    ...
    while ( add_mol ):
        ...
        while ( overlap_found ):
            ...
            if( overlap_sum ==  0 ):
                ...
                for pid, ptclObj in struc_i.ptclC :
                    ptclObj.tagsDict["chain"] += range_i_chain
...
def add_struc_ongrid(verbose,p,rank,molecule_rep,struc_i,n_sol_l,atomic_cut,lc_expand,max_sys,max_mol_place,calc_overlap,random_rotate=True,debug = True ):
    ...
    log_line = "\n max_rep_chain {} ".format(max_rep_chain)
    log_line += "\n range_i_chain {} ".format(range_i_chain)
    if( rank == 0 ):
        print log_line
    ...
    pointIndices = range( len(molecule_rep.ptclC)  )
    myChunk  = p.splitListOnProcs(pointIndices)
    ...
    while ( add_sol ):
        ...
        while ( overlap_found ):
                    ...
                    for z_indx in range(sol_box_side):
                        ...
                        if( overlap_sum ==  0 ):
                            ...
                            if( random_rotate ):
                                ...
                                if( debug ):
                                    print " Random rotation ",rank,rot_angle_j,rot_angle_j
                                    p.barrier() # Barrier for MPI_COMM_WORLD
                            ...
                            # if( debug): print sys_sol_n,lat_pos,center_mass(struc_i)
            ...
            if( debug):
                print " Attempts {}/{} ".format(strucadd_atempts,max_mol_place)
            if( strucadd_atempts >= max_mol_place ):
                # If attempts to place molecule into the system exceed max set by max_mol_place
                break
        ...
        if( sys_sol_n == n_sol_l ):
            ...
            sys_attempts += 1
            if( rank == 0  ):
                print '        - Placment failed  after adding {} particles and {}/{} attempts   '.format(sys_sol_n,sys_attempts,max_sys)
            ...
            if (  sys_attempts >= max_sys ):
                ...
                if( shrink_grid ):
                    ...
                    if(  (sol_grid_len*0.99) > sol_length):
                        ...
                        #if( verbose ):                
                        if( rank == 0  ):
                            print '          - Lattice vectors will be expanded by solvent grid length {} '.format(sol_length)
                else:
...
def main():
    ...
    # Initialize mpi
    p = mpiBase.getMPIObject(verbose=False)
    ...
    all_mol_struc = StructureContainer(verbose=False)
    replicate_struc = StructureContainer(verbose=False)
    replicate_prama = ParameterContainer()

    if( rank == 0 ):
        out_log = open("replicate.log",'w')

    if( options.in_param ):
        if( options.verbose ): print " Reading param file {} ".format(options.in_param)
        param_o  = read_param( options.in_param, verbose = True)

    ranmol_split = options.ranmol.split()
    print "ranmol_split",ranmol_split

    xN_list = []
    id_list = []
    for i in range(0,len(ranmol_split),2):        
        id_list.append(str(ranmol_split[i]))
        xN_list.append(int(ranmol_split[i+1]))
    ...
    if( len(id_list) > 0 ):
        for i in range(len(id_list)):
            ...
            struc_xN = xN_list[i]
            if( xN_list > 0 ):
                cply_file = "{}.cply".format(struc_id)
                if( options.verbose): print "Reading {}".format(cply_file)
                struc_i = StructureContainer(verbose=False)
                struc_i.read_cply(cply_file)
                if( rank == 0 ):
                    print str(struc_i)
                all_mol_struc += struc_i
                # Use biggest box size structure to set the box size 
                if( struc_i.getLatVec()[0][0] >  replicate_struc.getLatVec()[0][0]  ):
                    replicate_struc.setLatVec( struc_i.getLatVec() )
                struc_list.append(struc_i)
                
    if( options.lat > 0.0 ):
        replicate_struc.getLatVec()[0][0] = options.lat 
        replicate_struc.getLatVec()[1][1] = options.lat 
        replicate_struc.getLatVec()[2][2] = options.lat 

    if( len(options.grid_cply) > 0 and (options.grid_n > 0 or options.perc_grid > 0.0 ) ):
        grid_struc = StructureContainer(verbose=False)
        grid_struc.read_cply(options.grid_cply)
        if( rank == 0 ):
            print str(grid_struc)
        all_mol_struc += grid_struc

        
        if( options.perc_grid > 0.0 ):
            if( rank == 0 ):
                sol_n_old = options.grid_n
                if( rank == 0 ):
                    log_line = " Resetting the numnber of solvent molecules based on mass of molecules"
                    out_log.write(log_line)
                    if( options.verbose ):
                        print log_line
            ref_mol = 0 
            mol_n = xN_list[ref_mol] # options.mol1_n           
            mol_mass = struc_list[ref_mol].getTotMass()
            frac_sol = options.perc_grid/100.0
            sol_mass = grid_struc.getTotMass()
            #  frac_sol = grid_n*sol_mass/( mol_mass*mol_n +  grid_n*sol_mass) 
            #  frac_sol*( mol_mass*mol_n +  grid_n*sol_mass)  = grid_n*sol_mass
            #  frac_sol* mol_mass*mol_n +   frac_sol*grid_n*sol_mass  = grid_n*sol_mass
            #  frac_sol*grid_n*sol_mass  - grid_n*sol_mass = -  frac_sol* mol_mass*mol_n 
            #   grid_n* (sol_mass - frac_sol*sol_mass ) =   frac_sol* mol_mass*mol_n 
            #   grid_n =   frac_sol* mol_mass*mol_n /(sol_mass - frac_sol*sol_mass )
            #  grid_n  = mol_mass*mol_n*frac_sol / ( sol_mass -  sol_mass *frac_sol ) 
            #  grid_n * sol_mass (  1/ frac_sol  - 1 ) = mol_mass*mol_n 
            #  grid_n  = mol_mass*mol_n*frac_sol / ( sol_mass -  sol_mass *frac_sol ) 
            options.grid_n  = int( mol_mass*mol_n*frac_sol / ( sol_mass -  sol_mass *frac_sol ) )

            if( rank == 0 ):
                log_line = "\n  Number of grid molecules changed from %d to %d "%(sol_n_old,options.grid_n)
                log_line += "\n  which is {}% of mass of molecule {} and solvent mass ".format( round( 100.0*options.grid_n*sol_mass/( mol_mass*mol_n +options.grid_n*sol_mass), 1) ,ref_mol+1  ) 
                out_log.write(log_line)
                if( options.verbose ):
                    print log_line
        ...
        len_target = replicate_struc.getLatVec()[0][0]
        n_particles_sol = len(grid_struc.ptclC)*options.grid_n
        sol_box_side = int(math.ceil(options.grid_n**(1.0/3.0) ) )    # Number of solvents per box side 
        sol_maxlength = getlength(grid_struc) + 2.0*options.atomic_cut
        len_target_ang_min_sol = float(sol_box_side)*sol_maxlength
        #n_particles += n_particles_sol

        # Check to be sure the grid is large enough
        if( len_target_ang_min_sol > len_target ):
            if( rank == 0 ):
                log_line = "\n  The box size has been set to a length smaller than the box need to fit all the mol on grid"
                out_log.write(log_line)                
                if( options.verbose ):
                    print log_line
            options.min_grid_len = True

            
        if( options.min_grid_len  ):
            if( rank == 0 ):
                log_line = "\n  Changing the boxsize to {} the minimum need to fit all the solvents".format(len_target_ang_min_sol) 
                out_log.write(log_line)
                if( options.verbose ):
                    print log_line
            replicate_struc.setLatVec([np.array([len_target_ang_min_sol,0.0,0.0]),np.array( [0.0,len_target_ang_min_sol,0.0]),np.array( [0.0,0.0,len_target_ang_min_sol]) ])
    ...
    for i in range(len(id_list)):
         if( rank == 0 ):
             log_line = "System {} with name {} will be replicated {} times ".format(i,id_list[i],xN_list[i])
             out_log.write(log_line)
             print log_line
        # struc_id = id_list[i]
        # struc_xN = xN_list[i]
    ...
    for i in range(len(id_list)):
        struc_id = id_list[i]
        struc_xN = xN_list[i]
        struc_i = struc_list[i]
        if( xN_list > 1 ):
            replicate_struc = add_struc(options.verbose,p,rank,replicate_struc,struc_i,struc_xN,options.atomic_cut,options.lc_expand,options.max_sys,options.max_mol_place,options.calc_overlap,options.seed)
        elif( xN_list > 0 ):
            replicate_struc += struc_i
            
        if( rank == 0 ):
            log_line = "Finished system {} with name {} has been replicated {} times ".format(i,id_list[i],xN_list[i])
            out_log.write(log_line)
            print log_line

    if( len(options.grid_cply) > 0 and options.grid_n > 0 ):
        replicate_struc = add_struc_ongrid(options.verbose,p,rank,replicate_struc,grid_struc,options.grid_n,options.atomic_cut,options.lc_expand,options.max_sys,options.max_mol_place,options.calc_overlap)
        if( rank == 0 ):
            log_line = "Finished  system {} replicated {} on grid  ".format(options.grid_cply, options.grid_n)
            out_log.write(log_line)
            print log_line

    if( rank == 0 ):
        log_line = " Writing structure "
        out_log.write(log_line)
        if( options.verbose ):
            print log_line
        write_xmol(replicate_struc,"{}.xyz".format(options.out_id))
        replicate_struc.write_cply("{}.cply".format(options.out_id),write_ff=True,write_bonds=True)
