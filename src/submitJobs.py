...
%prog [options]
     ...
     Edit script notes:
     ##########################
     An example of the script edit command:

       submitJobs.py -t mols-cpdt-A1.DfA  -e '#PBS -l walltime=48:00:00 --> #PBS -l walltime=24:00:00'

     note the single quotes, the arrow and the spaces around the arrows!
...
def getJobInfo(td, ptn, chkQueue=False):
    ...
    for f in files:
        ...
        jobDone = js.isJobDone(jobPath, jobScript)       # Determine if job finished
        if (chkQueue):
            jobInQueue = js.isJobInQueue(jobPath, jobScript) # Determine if job queued
        else:
            jobInQueue = False  # Default value for not checking

        job.append(jobPath)             # .
        job.append(jobScript)           # ..
        job.append(jobDone)             # job = (test-runs/runDir1/runDir-1.1, job.mb, True/False)
        job.append(jobInQueue)          # True/False (if running or queued skip)
...
if (options_set):
    ...
    jobs = getJobInfo(topdir, searchPattern, True)
    numJobs = len(jobs)
    ...
    for j in jobs:
        ...
        jqueue = j[3]         # Job queue status

        # If job is in queue (ie running or queued) move on
        if (jqueue):
            notJobIO.prt(jdir + " is in queue... going to next job")
            continue
...
# Only check which jobs are done
    ...
    # List of 2-tuple (job location, job script)
    jobs = getJobInfo(topdir, searchPattern, False)
    numJobs = len(jobs)
    ...
    for j in jobs:
        ...
        jscr   = j[1]         # Job script
        jdone  = j[2]         # Job status
        # jqueue = j[3]         # Job queue status (NOT valid)
